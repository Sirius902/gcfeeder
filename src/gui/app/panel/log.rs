...
pub struct LogPanel {
    ...
    messages: Vec<(Message, usize, bool)>,
    auto_scroll: bool,
}
...
impl LogPanel {
    ...
    pub fn ui(&mut self, ui: &mut egui::Ui) {
        while let Ok(message) = self.receiver.try_recv() {
            if let Some((last_message, last_count, overflowed)) = self.messages.last_mut() {
            ...
            self.messages.push((message, 1, false));
        }
        ...
        egui::ScrollArea::both()
            ...
            .show_rows(ui, row_height, self.messages.len(), |ui, rows| {
                ...
                grid.show(ui, |ui| {
                    for (message, count, overflowed) in rows.map(|i| &self.messages[i]) {
                        ui.horizontal(|ui| {
                            message.draw(ui);
                            if *count != 1 {
                                let symbol = if *overflowed { ">" } else { "x" };
